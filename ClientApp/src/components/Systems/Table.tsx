    ...
    return (
        <React.Fragment>
            <thead>
                                ...
                                <Fragment key={indx}>
                                    {IsHasPermission || !data.IsCheckBox ?
                                        <th colSpan={data.Colspan} key={indx} rowSpan={data.Rowspan}
                                            style={data.CSSStyle}
                                            className={`${!!data.ClassName ? data.ClassName : ""} ${data.Sortby ? "sorting" : ""}`}
                ...
                {
                    Header2 === "4" ? <tr>
                        <td className="text-center" style={{ width: 180 }}>เพิ่ม</td>
                        <td className="text-center" style={{ width: 180 }}>ลด</td>
                    </tr> : true
                }
            </thead>
        </React.Fragment>
...
const TableFooter = (Props: { createFooterRow: any }) => {
    if (!Props.createFooterRow) return null
    const a = Props.createFooterRow
    return (       
        <>
            {Props.createFooterRow()}
        </>
    )
}

export const CreateTable = (Props: {
    Header: CellHeader[], Header2?: any, ItemData: Array<any>, CreateDataRow: any, onBtnDelClick?: any, IsHasPermission?: any, IsNoPaging?: any
    , IsHasBtnDEL?: Boolean, onClickHeadCB?: any, rowSelected?: any | [], TableClassName?: any, TableStyles?: CSSProperties, defaultFontSize?: boolean
    , defaultCancel?: boolean, createFooterRow?: any,
}) => {//
    const { Header, Header2, ItemData, CreateDataRow, IsHasBtnDEL, onBtnDelClick, onClickHeadCB, rowSelected, TableClassName, TableStyles, IsHasPermission = true, IsNoPaging, defaultFontSize, defaultCancel, createFooterRow } = Props
    ...
    var Paging = currentData.length !== data.length ?
        <Col className="col-auto" md="auto" xs="12" sm="auto">
            <Pagination
                hideDisabled
    ...
    return (
        <React.Fragment>
            <Col sm='12'>
                <div className={currentData.length === 0 ? "" : ""} style={currentData.length === 0 ? { display: "block", width: "100%", overflowX: "hidden", overflowY: "auto" } : { display: "block", width: "100%", marginBottom: "1rem", overflowX: "unset", overflowY: "auto", borderBottom: "1px solid #dee2e6" }}>
                    <Table size="sm" responsive={currentData.length !== 0 ? true : false} className={(TableClassName || "bg-white mb-0") + (defaultFontSize ? "" : " small")} style={TableStyles}>
                        ...
                        <TableFooter createFooterRow={createFooterRow} />
                    </Table>
                </div>
                {DataNotFound}
            </Col>
            ...
            {data.length !== 0 && <Fragment>
                ...
                {
                    IsHasBtnDEL && data.length !== 0 &&
                    (IsHasPermission &&
                        <Col className="col-auto"><span data-tip={TooltipsMSG.Delete}>
                            <Button disabled={rowSelected && rowSelected.length === 0}
                                size="sm" onClick={onBtnDelClick} type="button" className="btn btn-danger">
                                {
                                    defaultCancel ? (<i className="fas fa-times"> ยกเลิก</i>) : <FontAwesomeIcon icon={['fas', 'trash-alt']} />
                                }
                            </Button></span>
                        </Col>)
                }

                <Col className="col-auto ml-auto" style={IsNoPaging === true ? { display: "none" } : {}}>
                    <Row form>
                        {Paging}
                        {data.length >= 10 &&
                            <Col className="col-auto">
                                <FormGroup>
                                    <select className="form-control form-control-sm" onChange={onChangeDataPerPage}>
                                        <option value="10">10</option>
                                        <option value="20">20</option>
                                        <option value="50">50</option>
                                        <option value="100">100</option>
                                        <option value={data.length}>ทั้งหมด</option>
                                    </select>
                                </FormGroup>
                            </Col>
                        }
